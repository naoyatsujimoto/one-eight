-- =============================================================================
-- Phase D-2 reapply: process_arena_results() GREATEST保護を確実に適用
--
-- 背景:
--   20260606233738_arena_phase_d2_fix_arena_id.sql は CLIにより適用済みと
--   判断されているが、arena_points.points へのGREATEST(points + delta, 0)保護が
--   実際のDB上の関数定義に反映されていなかった。
--   本migrationで CREATE OR REPLACE で強制再適用する。
-- =============================================================================

DROP FUNCTION IF EXISTS process_arena_results();

-- ================================================================
-- process_arena_results() 完全実装 (GREATEST保護込み)
-- ================================================================
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      --    GREATEST保護: points が 0 未満にならないよう保護
      -- ----------------------------------------------------------------
