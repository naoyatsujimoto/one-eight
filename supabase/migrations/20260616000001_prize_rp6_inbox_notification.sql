-- ============================================================
-- RP-6 patch: admin_generate_arena_prize_awards
-- 新規 prize_award 作成時に admin_messages へ個人向け通知を追加
--
-- 変更点:
--   新規 prize_award 作成（ELSE ブランチ）のみ、
--   対象ユーザー個人宛ての admin_messages レコードを1件作成する。
--
-- 重複防止:
--   admin_messages.source_id = v_award_id::text + target = recipient::text
--   の UNIQUE INDEX により、同一 Award の二重通知を防ぐ。
--
-- already_exists 時:
--   admin_messages への INSERT は行わない。
--   （再実行でメールが増殖しない）
--
-- 維持する仕様:
...
--   - match_kind = 'master'
--   - no_contest / cancelled 除外
--   - 既存 prize_awards の重複防止（変更なし）
--   - target = 'all' にはしない（個人向けのみ）
-- ============================================================
