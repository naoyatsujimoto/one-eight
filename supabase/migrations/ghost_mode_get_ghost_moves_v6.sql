-- =============================================================================
-- Migration: Ghost Mode — get_ghost_moves RPC v6
-- v5からの変更: ghost_candidates の jsonb_array_length を完全削除
-- 理由: CASE WHEN でもPostgreSQLが両辺を評価し22023が発生する
-- out-of-bounds の -> integer は NULL を返すため downstream の IS NOT NULL で処理
-- =============================================================================
...
BEGIN
  ...
  ghost_candidates AS (
    ...
    -- out-of-bounds の場合 -> は NULL を返す（下の IS NOT NULL フィルタで除去）
