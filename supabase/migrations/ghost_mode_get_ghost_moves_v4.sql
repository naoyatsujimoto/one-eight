-- =============================================================================
-- Migration: Ghost Mode — get_ghost_moves RPC v4
-- 変更内容:
--   v3からの修正: jsonb_array_length を完全除去
--   - target_logs: jsonb_array_length → jsonb_typeof + != '[]' に変更
--   - ghost_candidates: jsonb_array_length 除去（OUT OF BOUNDS は downstream NULL フィルタで処理）
--   - target_logs を MATERIALIZED 化（PostgreSQL 12+ CTE インライン展開対策）
-- =============================================================================
...
BEGIN
  ...
  RETURN QUERY
  WITH
  -- MATERIALIZED: CTE インライン展開を防ぎ、jsonb_typeof フィルタを必ず先に評価させる
  target_logs AS MATERIALIZED (
  ...
  ghost_candidates AS (
    ...
    -- OUT OF BOUNDS の場合 -> は NULL を返す（downstream の ghost_move IS NOT NULL で除去）
