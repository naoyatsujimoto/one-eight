-- =============================================================================
-- Phase D-2 fix: process_arena_results() の arena_id 参照を arena_events JOINに変更
-- arena_matches に arena_id 列はなく、arena_event_id → arena_events.arena_id でJOIN
-- =============================================================================

DROP FUNCTION IF EXISTS process_arena_results();

-- =============================================================================
-- Official Arena Phase D-2 — process_arena_results() RPC
--
-- 前提:
--   D-1.5 schema alignment 完了済み（arena_matches.status IN (processed追加済み),
--          arena_points UNIQUE INDEX(arena_id,user_id,season)追加済み,
--          arena_match_history 1 match 1 row形式で再作成済み）
--
-- 注意:
--   - arena_events.results_processed_at 列が未存在のため本migrationで追加する
--     (matches_generated_at と同様のパターン)
--   - arena_matches に arena_id 列はなく、arena_event_id → arena_events.arena_id
--     でArenaに紐づくため、JOINで取得する
-- =============================================================================

-- ================================================================
-- 0. arena_events に results_processed_at 列追加
-- ================================================================

-- ================================================================
-- 1. 既存の process_arena_results 関数があれば削除
-- ================================================================

-- ================================================================
-- 2. process_arena_results() 実装
-- ================================================================
    ...
    BEGIN
      ...
      THEN
        IF v_om.winner = 'black_user' THEN
          ...
          v_black_delta      := 3;
          v_white_delta      := 1;  -- timeout敗者も +1（-3にしない）
        ELSIF v_om.winner = 'white_user' THEN
          v_winner_uid       := r_match.white_user_id;
          v_loser_uid        := r_match.black_user_id;
          v_arena_end_reason := v_om.end_reason;
          v_black_delta      := 1;  -- timeout敗者も +1（-3にしない）
          v_white_delta      := 3;
        ELSE
      ...
      IF v_arena_end_reason NOT IN ('no_contest','cancelled') THEN
        ...
        INSERT INTO arena_points (
          ...
          'default',
          GREATEST(v_black_delta, 0),  -- 新規エントリでも0未満にしない
        ...
        DO UPDATE SET
          points         = GREATEST(arena_points.points + v_black_delta, 0),
        ...
        INSERT INTO arena_points (
          ...
          'default',
          GREATEST(v_white_delta, 0),  -- 新規エントリでも0未満にしない
        ...
        ON CONFLICT (arena_id, user_id, season)
        DO UPDATE SET
          points         = GREATEST(arena_points.points + v_white_delta, 0),
          win_count      = arena_points.win_count
                           + CASE WHEN v_winner_uid = r_match.white_user_id THEN 1 ELSE 0 END,
          loss_count     = arena_points.loss_count
                           + CASE WHEN v_loser_uid = r_match.white_user_id
