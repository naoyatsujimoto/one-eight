-- =============================================================================
-- Migration: fix(history): include online matches for both players
--
-- 目的:
--   時間切れ対局で、タイムアウト確定RPCを呼んだ側（match_logs.user_id）しか
--   履歴に含まれない問題を修正する。
--
-- 原因:
--   get_user_match_history() が match_logs.user_id = auth.uid() の行のみ返す。
--   online対局では相手が確定RPCを呼んだ場合、自分の対局が欠落する。
--
-- 修正方針:
--   - match_logsを参加者別に複製しない（1対局1行を維持）
--   - online_gamesの両参加者（black/white）がいずれかであれば対象に含める
--   - match_logs.game_id = online_games.id::text で結合（安全なテキスト比較）
--   - auth.uid()視点のhuman_colorを計算して返す
--   - 同じmatch_logは1回だけ返す（DISTINCT相当）
--   - Pro判定: F-05と同一（active + canceled期限内）
--   - Free: created_at降順で直近10件（参加者条件適用後）
--
-- get_public_match_logs(target_user_id) も同様の参加者基準に修正する:
--   - target_user_idのstats_public=trueの場合のみ返す
--   - canonical match_logs所有者ではなく、閲覧対象ユーザーの公開設定を確認
--   - target_user_id視点のhuman_colorを返す
--   - 既存の最大100件・降順を維持
--
-- 禁止事項の遵守:
--   - match_logsの複製なし
--   - game_id一意制約の変更なし
--   - position statsへの二重集計なし
--   - 過去データ手動補正なし
-- =============================================================================


-- =============================================================================
-- 1. get_user_match_history() — 両参加者を含む形に修正
-- =============================================================================
DROP FUNCTION IF EXISTS get_user_match_history();

CREATE OR REPLACE FUNCTION get_user_match_history()
RETURNS TABLE (
  id                      UUID,
  user_id                 UUID,
  game_id                 TEXT,
  started_at              TIMESTAMPTZ,
  ended_at                TIMESTAMPTZ,
  mode                    TEXT,
  human_color             TEXT,
  winner                  TEXT,
  move_count              INTEGER,
  created_at              TIMESTAMPTZ,
  full_record             JSONB,
  cpu_difficulty          TEXT,
  canonical_hashes_computed BOOLEAN,
  timer_config            JSONB,
  end_reason              TEXT
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_uid        UUID;
  v_plan       TEXT;
  v_status     TEXT;
  v_period_end TIMESTAMPTZ;
  v_is_pro     BOOLEAN;
BEGIN
  v_uid := auth.uid();

  -- プラン取得
  SELECT plan, subscription_status, current_period_end
  INTO v_plan, v_status, v_period_end
  FROM profiles
  WHERE id = v_uid;

  -- F-05と同一のPro判定: active + canceled期限内
  v_is_pro := (
    v_plan = 'pro'
    AND (
      (v_status = 'active'   AND (v_period_end IS NULL OR v_period_end > now()))
      OR
      (v_status = 'canceled' AND v_period_end IS NOT NULL AND v_period_end > now())
    )
  );

  IF v_is_pro THEN
    -- 有料: 対象となる全履歴
    RETURN QUERY
      SELECT
        m.id,
        m.user_id,
        m.game_id,
        m.started_at,
        m.ended_at,
        m.mode,
        -- online対局でauth.uid()が参加者の場合、視点に応じたhuman_colorを返す
        CASE
          WHEN m.mode = 'online_pvp' AND og.id IS NOT NULL THEN
            CASE
              WHEN og.black_player_id = v_uid THEN 'black'
              WHEN og.white_player_id = v_uid THEN 'white'
              ELSE m.human_color
            END
          ELSE m.human_color
        END AS human_color,
        m.winner,
        m.move_count,
        m.created_at,
        m.full_record,
        m.cpu_difficulty,
        m.canonical_hashes_computed,
        m.timer_config,
        m.end_reason
      FROM match_logs m
      LEFT JOIN online_games og
        ON m.mode = 'online_pvp'
        AND m.game_id = og.id::text
        AND (og.black_player_id = v_uid OR og.white_player_id = v_uid)
      WHERE
        -- 従来: match_logs.user_id = auth.uid()
        m.user_id = v_uid
        -- 追加: online対局でauth.uid()がblack/whiteの参加者（タイムアウト確定側でない場合）
        OR (m.mode = 'online_pvp' AND og.id IS NOT NULL)
      ORDER BY m.created_at DESC;
  ELSE
    -- 無料: 対象となる履歴をcreated_at降順で直近10件
    RETURN QUERY
      SELECT
        m.id,
        m.user_id,
        m.game_id,
        m.started_at,
        m.ended_at,
        m.mode,
        CASE
          WHEN m.mode = 'online_pvp' AND og.id IS NOT NULL THEN
            CASE
              WHEN og.black_player_id = v_uid THEN 'black'
              WHEN og.white_player_id = v_uid THEN 'white'
              ELSE m.human_color
            END
          ELSE m.human_color
        END AS human_color,
        m.winner,
        m.move_count,
        m.created_at,
        m.full_record,
        m.cpu_difficulty,
        m.canonical_hashes_computed,
        m.timer_config,
        m.end_reason
      FROM match_logs m
      LEFT JOIN online_games og
        ON m.mode = 'online_pvp'
        AND m.game_id = og.id::text
        AND (og.black_player_id = v_uid OR og.white_player_id = v_uid)
      WHERE
        m.user_id = v_uid
        OR (m.mode = 'online_pvp' AND og.id IS NOT NULL)
      ORDER BY m.created_at DESC
      LIMIT 10;
  END IF;
END;
$$;

-- authenticated のみ実行可能
GRANT EXECUTE ON FUNCTION get_user_match_history() TO authenticated;
REVOKE EXECUTE ON FUNCTION get_user_match_history() FROM anon;


-- =============================================================================
-- 2. get_public_match_logs(target_user_id) — 両参加者を含む形に修正
-- =============================================================================
DROP FUNCTION IF EXISTS get_public_match_logs(UUID);

CREATE OR REPLACE FUNCTION get_public_match_logs(target_user_id UUID)
RETURNS TABLE (
  id                      UUID,
  user_id                 UUID,
  game_id                 TEXT,
  started_at              TIMESTAMPTZ,
  ended_at                TIMESTAMPTZ,
  mode                    TEXT,
  human_color             TEXT,
  winner                  TEXT,
  move_count              INTEGER,
  created_at              TIMESTAMPTZ,
  full_record             JSONB,
  cpu_difficulty          TEXT,
  canonical_hashes_computed BOOLEAN,
  timer_config            JSONB,
  end_reason              TEXT
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_is_public BOOLEAN;
BEGIN
  -- 閲覧対象ユーザーのstats_publicを確認
  -- canonical match_logs所有者ではなく、target_user_idの設定を参照
  SELECT stats_public INTO v_is_public
  FROM profiles
  WHERE id = target_user_id;

  IF NOT FOUND OR NOT v_is_public THEN
    -- stats_public = false または ユーザー不存在 → 空を返す
    RETURN;
  END IF;

  RETURN QUERY
    SELECT
      m.id,
      m.user_id,
      m.game_id,
      m.started_at,
      m.ended_at,
      m.mode,
      -- target_user_id視点のhuman_colorを返す
      CASE
        WHEN m.mode = 'online_pvp' AND og.id IS NOT NULL THEN
          CASE
            WHEN og.black_player_id = target_user_id THEN 'black'
            WHEN og.white_player_id = target_user_id THEN 'white'
            ELSE m.human_color
          END
        ELSE m.human_color
      END AS human_color,
      m.winner,
      m.move_count,
      m.created_at,
      m.full_record,
      m.cpu_difficulty,
      m.canonical_hashes_computed,
      m.timer_config,
      m.end_reason
    FROM match_logs m
    LEFT JOIN online_games og
      ON m.mode = 'online_pvp'
      AND m.game_id = og.id::text
      AND (og.black_player_id = target_user_id OR og.white_player_id = target_user_id)
    WHERE
      -- target_user_idが直接所有する match_log
      m.user_id = target_user_id
      -- online対局でtarget_user_idが参加者（match_logs所有者でない場合）
      OR (m.mode = 'online_pvp' AND og.id IS NOT NULL)
    ORDER BY m.created_at DESC
    LIMIT 100;
END;
$$;

-- anon・authenticated 両方に公開（公開プロフィール設計）
GRANT EXECUTE ON FUNCTION get_public_match_logs(UUID) TO anon;
GRANT EXECUTE ON FUNCTION get_public_match_logs(UUID) TO authenticated;
