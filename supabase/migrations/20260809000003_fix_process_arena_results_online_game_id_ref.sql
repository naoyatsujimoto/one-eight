-- =============================================================================
-- Fix: process_arena_results() — Pass 1 の online_game_id 参照先修正
--
-- 問題:
--   20260809000001 の Pass 1 カーソルで arena_matches.online_game_id を参照しているが、
--   本番データでは arena_matches.online_game_id = NULL（全11件で0件）。
--   online_game_id は official_matches.online_game_id に格納されている。
--
-- 修正内容:
--   Pass 1 カーソルの SELECT で am.online_game_id を
--   om.online_game_id AS online_game_id に変更する。
--
-- 変更しないもの:
--   - Pass 2 の全ロジック
--   - 修正2（FOUND フラグ）・修正3（defend 判定）
--   - online_games 更新ロジック（forfeit_white / forfeit_black）
--   - ポイント計算・Master 更新・arena_events 処理
--   - SECURITY DEFINER / SET search_path / GRANT-REVOKE パターン
-- =============================================================================
...
DECLARE
  ...
  -- FIX: Pass 1 で online_game_id を保持する変数（om.online_game_id を使用）
...
BEGIN
  ...
  FOR r_exp IN
    SELECT
      ...
      -- FIX: am.online_game_id ではなく om.online_game_id を参照する
      -- 本番データでは arena_matches.online_game_id = NULL のため
      om.online_game_id       AS online_game_id,
      am.arena_event_id,
      om.starts_at,
      om.timer_config,
      om.black_user_id,
  ...
  LOOP
    ...
    v_online_game_id := r_exp.online_game_id;  -- FIX: om.online_game_id から取得
    ...
    -- online_game が存在し playing 状態の場合も終了させる
    ...
    BEGIN
      ...
      IF r_match.match_kind = 'master' THEN
        ...
        ELSIF v_winner_uid IS NOT NULL THEN
          ...
          -- FOUND フラグを使って active master/interim を安全に取得
