-- =============================================================================
-- Fix: process_arena_results() — defend / master_succession / interim_set branch
--      で arena_master_active_official_uniq 制約違反が発生する問題を修正
--
-- 原因:
--   defend / master_succession / interim_set branch での arena_master_history UPDATE が
--   WHERE id = v_active_master.id に依存していた。
--   v_active_master が SELECT で取得できなかった場合（RLS / 可視性の問題等）、
--   UPDATE がスキップされ、旧 active official row が残ったまま INSERT が実行される。
--   → arena_master_active_official_uniq (UNIQUE ON arena_id, season WHERE status='official'
--     AND dethroned_at IS NULL) 違反で INSERT が失敗。
--   → EXCEPTION WHEN OTHERS に吸収 → v_errors に格納されサイレント失敗。
--   → pg_cron は成功と認識し続けるため、以降も処理されない。
--
-- 修正内容:
--   1. defend branch, master_succession branch, interim_set branch の
--      UPDATE condition を id依存から arena_id/season 依存に変更。
--      確実に active row を失冠してから INSERT するようにする。
--   2. EXCEPTION handler に RAISE WARNING を追加し、pg_cron ログでエラーが見えるようにする。
--
-- 変更しないもの:
--   - 関数名・引数・戻り値形式
...
--   - SECURITY DEFINER
--   - GRANT/REVOKE パターン
--   - inaugural / defended / no_contest / no_show / point 処理ロジック
--   - arena_match_history INSERT / arena_matches UPDATE
--   - Pass 1 (no-show resolve) ロジック
--   - Step 10 (arena_events completed 化)
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      -- FIX: RAISE WARNING でpg_cronログにエラーが見えるようにする（機微情報含まず）
