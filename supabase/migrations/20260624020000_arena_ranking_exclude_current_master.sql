-- Fix get_arena_detail: top_ranking から現在の active Master を除外する
-- arena_master_history で dethroned_at IS NULL のユーザーをランキング集計から除外
...
BEGIN
  ...
  -- top ranking: 直近90日の arena_match_history から集計
  -- 現在の active Master（dethroned_at IS NULL）を除外する
