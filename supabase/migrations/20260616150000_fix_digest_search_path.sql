-- ============================================================
-- Fix: admin_prepare_payout の digest() 解決エラー修正
--
-- 問題: SET search_path = public により extensions スキーマの
--        pgcrypto digest() 関数が解決できない。
--        エラー: function digest(text, unknown) does not exist
--
-- 修正: search_path に extensions を追加
--        SET search_path = public, extensions
-- ============================================================

-- pgcrypto 有効化（extensions スキーマ指定）
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- admin_prepare_payout を search_path 修正で再定義
...
COMMENT ON FUNCTION admin_prepare_payout(uuid) IS
  'Admin: prepare a payout for an eligible award. Snapshots payment info from submission_data. Returns payout_id without PII. Fix: search_path includes extensions for pgcrypto digest() (2026-06-16)';
