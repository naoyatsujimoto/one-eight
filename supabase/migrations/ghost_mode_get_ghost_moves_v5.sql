-- =============================================================================
-- Migration: Ghost Mode — get_ghost_moves RPC v5
-- v4からの変更:
--   AS MATERIALIZED を廃止（plpgsql 環境での互換性問題を回避）
--   代わりに CASE WHEN で jsonb_array_length 呼び出しを安全にガード
--   ghost_candidates の jsonb_array_length も CASE WHEN でガード
-- =============================================================================
...
BEGIN
  ...
  ghost_candidates AS (
    ...
    WHERE p_move_index > 0
      ...
      AND elem.ord::int < CASE
            WHEN jsonb_typeof(tl.full_record) = 'array'
            THEN jsonb_array_length(tl.full_record)
            ELSE 0
          END
  ),
  filtered AS (
