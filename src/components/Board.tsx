...
const GATE_LABEL_OFFSET: Record<GateType, { left: number; top: number }> = {
  ...
  'corner-tl':   { left: 4, top: 4 },  // 左上コーナー（中心方向へ+20px）
  'corner-tr':   { left: 86, top: 4 },  // 右上コーナー（中心方向へ+20px）
  'corner-br':   { left: 86, top: 86 },  // 右下コーナー（中心方向へ+20px）
  'corner-bl':   { left: 4, top: 86 },  // 左下コーナー（中心方向へ+20px）
};
...
export function Board({
  ...
  useEffect(() => {
    ...
    for (const gateId of relatedGates) {
      ...
      // Corner gates: target the position-facing corner of the bounding rect.
      let tx: number;
      let ty: number;
      switch (gateType) {
        case 'top-edge':
      ...
      // Shorten line so it stops just before reaching the gate
      const dx = tx - posX;
      const dy = ty - posY;
