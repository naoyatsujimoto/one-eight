/**
 * import_fashard_fh01.ts
 *
 * Fast Hard vs Fast Hard 10,000局 を sim_match_logs に取り込む。
 * sim_batch_id: fahard_20260515_001
 ...
 *   - Phase C: 別スクリプト phase_c_fashard_fh01.ts で実施
 *   - Phase D: 別スクリプト phase_d_posonly_fashard_fh01.ts で実施
 *
 * 実行方法:
 *   nohup npx vite-node scripts/import_fashard_fh01.ts > /tmp/fh01_import.log 2>&1 &
...
const SIM_BATCH_ID        = 'fahard_20260515_001';
...
async function main() {
  ...
  log('\n→ 次: phase_c_fashard_fh01.ts を実行して sim_medium_pattern_stats を更新');
  log('→ 次: phase_d_posonly_fashard_fh01.ts を実行して sim_position_only_stats を更新');
  log('=== Phase A 完了 ===');
}
