/**
 * import_fvhard_008.ts
 *
 * Fast-Very-Hard vs Fast-Very-Hard 10,000局 を sim_match_logs に取り込む。
 * sim_batch_id: fvhard_20260702b_008
 * sim_policy  : fastveryhard_vs_fastveryhard
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   nohup node_modules/.bin/vite-node /tmp/import_fvhard_008.ts > /tmp/fvhard006_import.log 2>&1 &
 *   tail -f /tmp/fvhard006_import.log
 *
 * 制約:
...
const SIM_FILE_PATH       = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_fastveryhard_vs_fastveryhard_20260702b.md';
const SIM_BATCH_ID        = 'fvhard_20260702b_008';
const SIM_POLICY          = 'fastveryhard_vs_fastveryhard';
const SIM_VERSION         = '1.0.0';
const ENGINE_VERSION      = '1.0.0';
const RULES_VERSION       = '1.1.0';
const GENERATED_AT        = '2026-07-02T00:00:00Z';
...
function replayGame(g: ParsedGame): ExtMoveRecord[] {
  ...
  for (const raw of g.moves) {
    ...
    if (raw.buildType === 'pass') {
      ...
      result.push({
        moveNumber: moveNum, player: moveNum % 2 === 1 ? 'black' : 'white', positioning: 'P',
        build: { type: 'skip' },
    ...
    try {
      ...
      if (raw.buildType === 'm') {
        state = raw.gates.length > 0 ? applyMassiveBuild(state, raw.gates[0] as GateId) : confirmPositionOnly(state);
      } else if (raw.buildType === 's') {
        if (raw.gates.length >= 2) state = applySelectiveBuild(state, [raw.gates[0] as GateId, raw.gates[1] as GateId]);
        else if (raw.gates.length === 1) state = applySelectiveBuildSingle(state, raw.gates[0] as GateId);
        else state = confirmPositionOnly(state);
...
async function main() {
  ...
  log(`パース完了: ${games.length} ゲーム（棋譜あり）`);
  ...
  if (exist >= EXPECTED_GAME_COUNT) {
    ...
    if (phaseAErrors > 0) {
      log(`[ERROR] Phase A エラー ${phaseAErrors} 件 — 取り込み中止`);
      process.exit(1);
    }
  }
  ...
  const {count: easyAfter} = await supabase.from('sim_match_logs')
    .select('*',{count:'exact',head:true}).eq('sim_policy','easy_vs_easy');
  log(`sim_match_logs(fast_hard): ${fhAfter} (${fhBefore===fhAfter?'✅ 変化なし':'❌ 汚染'})`);
  ...
  if (batchCnt !== EXPECTED_GAME_COUNT) {
    log(`[ERROR] 取込件数不一致: ${batchCnt} ≠ ${EXPECTED_GAME_COUNT}`);
    process.exit(1);
  }

  log('\n→ 次: phase_c_fvhard_008.ts を実行');
  log('=== Phase A 完了 ===');
}
