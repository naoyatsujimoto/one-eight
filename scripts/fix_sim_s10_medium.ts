/**
 * fix_sim_s10_medium.ts
 *
 * import_sim_easy_s10.ts の Phase C 補完スクリプト（RPCなし・直接upsert版）。
 * easy_20260512_010 の full_record から medium_pattern_id を抽出し、
 * メモリ上で集計→既存データとマージ→バルクupsert。
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   export $(grep -v '^#' .env | xargs)
 *   npx vite-node scripts/fix_sim_s10_medium.ts
 */
...
async function main() {
  console.log('=== fix_sim_s10_medium.ts (bulk upsert版) ===');
  ...
  // ─── Step 1: easy_20260512_010 の全レコードをページング取得 ───
  console.log('Step 1: easy_20260512_010 データ取得中...');
