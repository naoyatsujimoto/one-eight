/**
 * import_fashard_fh10.ts
 *
 * Fast Hard vs Fast Hard 10,000局 を sim_match_logs に取り込む。
 * sim_batch_id: fahard_20260520_010
 * sim_policy  : fast_hard_vs_fast_hard
 *
 * ─────────────────────────────────────────────────
 * 棋譜フォーマット（テーブル形式・全手順）:
 *   "| # | 勝者 | 手数 | 黒Pos | 白Pos | 棋譜（全手順） |"
 *   区切り: スペース（" "）
 *   passターン: "P,pass"
 *   ファイル: sim_fashard_vs_fashard_20260520j.md（Run 10 棋譜あり版）
 *
 * 設計方針:
 *   - Phase A: sim_match_logs に UPSERT（conflict on sim_batch_id, game_index）
 *     ストリーミング処理（全量メモリ保持禁止）
 *   - Phase B: 廃止（sim_position_stats テーブル削除済み）
 *   - Phase C: 別スクリプト tmp/phase_c_fashard_fh10.ts で実施
 *   - Phase D: 別スクリプト tmp/phase_d_posonly_fashard_fh10.ts で実施
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   nohup node_modules/.bin/vite-node scripts/tmp/import_fashard_fh10.ts > /tmp/fh10_import.log 2>&1 &
 *   tail -f /tmp/fh10_import.log
 *
 * 制約:
 *   - match_logs / position_stats / medium_pattern_stats（実戦）への書き込み禁止
 *   - sim_position_stats への書き込み禁止（テーブル削除済み）
 *   - easy_vs_easy データとは絶対に混ぜない
 *   - full_record=[] のデータは取り込まない（棋譜なし行は除外）
 */
...
const SIM_FILE_PATH       = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_fashard_vs_fashard_20260520j.md';
const SIM_BATCH_ID        = 'fahard_20260520_010';
const SIM_POLICY          = 'fast_hard_vs_fast_hard';
const SIM_VERSION         = '1.0.0';
const ENGINE_VERSION      = '1.0.0';
const RULES_VERSION       = '1.1.0';
const GENERATED_AT        = '2026-05-20T12:00:00Z';
...
type ParsedGame = {
  gameIndex: number;       // 通し番号（1-indexed）
  winner: string;          // 'black' | 'white' | 'draw'
  moveCount: number;       // 記録上の総手数
  moves: RawMove[];        // 全手順
};

type ExtMoveRecord = MoveRecord & { medium_pattern_id?: string };

// ─── パーサー（テーブル行形式: スペース区切り全手順）────────────────────────

/**
 * 棋譜トークン "E,m(2)" / "M,q(1,6,7,8)" / "P,pass" をパース
 */
...
function parseSimFile(content: string): ParsedGame[] {
  ...
  for (const line of content.split('\n')) {
    ...
    // full_record=[] になる（棋譜なし）ゲームは除外
    if (moves.length === 0) continue;

    games.push({ gameIndex: globalIndex, winner, moveCount, moves });
  }
...
async function main() {
  ...
  log('[注意] 棋譜形式: テーブル行・全手順あり（full_record に全棋譜格納）\n');
  ...
  log('\n→ 次: phase_c_fashard_fh10.ts を実行して sim_medium_pattern_stats を更新');
  log('→ 次: phase_d_posonly_fashard_fh10.ts を実行して sim_position_only_stats を更新');
  log('=== Phase A 完了 ===');
}
