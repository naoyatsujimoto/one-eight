/**
 * phase_c_fashard_fh10.ts
 *
 * sim_batch_id=fahard_20260520_010 の sim_medium_pattern_stats への差分 upsert。
 * import_fashard_fh10.ts（Phase A）完了後に実行する。
 *
 * 設計:
 *   - fahard_20260520_010 のみをスキャン（全量メモリ保持禁止）
...
async function main() {
  ...
  if (!batchCount || batchCount === 0) {
    log('ERROR: バッチが sim_match_logs に存在しません。先に import_fashard_fh10.ts を実行してください。');
    process.exit(1);
  }
  ...
  log('\n→ 次: phase_d_posonly_fashard_fh10.ts を実行して sim_position_only_stats を更新');
  log('=== Phase C 完了 ===');
}
