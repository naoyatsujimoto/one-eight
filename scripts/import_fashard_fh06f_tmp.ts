/**
 * import_fashard_fh06f.ts
 *
 * Fast Hard vs Fast Hard 10,000局 を sim_match_logs に取り込む。
 * sim_batch_id: fahard_20260519_006
 ...
 *   区切り: スペース（" "）
 *   passターン: "P,pass"
 *   ファイル: sim_fashard_vs_fashard_20260519f.md（Run 6 棋譜あり版）
 *
 * 設計方針:
 ...
 *   nohup node_modules/.bin/vite-node scripts/archive/import_fashard_fh06f.ts > /tmp/fh06f_import.log 2>&1 &
 *   tail -f /tmp/fh06f_import.log
 *
 * 制約:
...
const SIM_FILE_PATH       = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_fashard_vs_fashard_20260519f.md';
const SIM_BATCH_ID        = 'fahard_20260519_006';
...
async function main() {
  log('=== import_fashard_fh06f.ts 開始 ===');
  ...
  log('\n→ 次: phase_c_fashard_fh06f.ts を実行して sim_medium_pattern_stats を更新');
  log('→ 次: phase_d_posonly_fashard_fh06f.ts を実行して sim_position_only_stats を更新');
  log('=== Phase A 完了 ===');
}
