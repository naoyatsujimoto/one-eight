/**
 * phase_d_fvhard_006.ts
 *
 * sim_batch_id=fvhard_20260702b_008 の sim_position_only_stats への差分 upsert。
 * phase_c_fvhard_008.ts 完了後に実行する。
 */
...
async function main() {
  ...
  if (!batchCount || batchCount === 0) {
    log('ERROR: バッチが sim_match_logs に存在しません。先に import_fvhard_008.ts を実行してください。');
    process.exit(1);
  }
  ...
  if (upsertErr > 0) {
    log(`[ERROR] Phase D upsert エラー ${upsertErr} 件 — 確認が必要`);
    process.exit(1);
  }
  ...
  const {count: easyPosAfter} = await supabase.from('sim_position_only_stats')
    .select('*',{count:'exact',head:true}).eq('sim_policy','easy_vs_easy');
  log(`sim_position_only_stats(fast_hard): ${fhPosAfter} (${fhPosBefore===fhPosAfter?'✅ 変化なし':'❌ 汚染'})`);
