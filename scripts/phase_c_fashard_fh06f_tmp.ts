/**
 * phase_c_fashard_fh06f.ts
 *
 * sim_batch_id=fahard_20260519_006 の sim_medium_pattern_stats への差分 upsert。
 * import_fashard_fh06f.ts（Phase A）完了後に実行する。
 *
 * 設計:
 *   - fahard_20260519_006 のみをスキャン（全量メモリ保持禁止）
 ...
 *   nohup node_modules/.bin/vite-node scripts/archive/phase_c_fashard_fh06f.ts > /tmp/fh06f_phase_c.log 2>&1 &
 *   tail -f /tmp/fh06f_phase_c.log
 */
...
const SKIP_THRESHOLD = 9000;  // スキップ9000件以上は異常
...
async function main() {
  ...
  if (!batchCount || batchCount === 0) {
    log('ERROR: バッチが sim_match_logs に存在しません。先に import_fashard_fh06f.ts を実行してください。');
    process.exit(1);
  }
  ...
  if (skipCount >= SKIP_THRESHOLD) {
    log(`\n❌ 異常: skipCount=${skipCount} >= ${SKIP_THRESHOLD}。処理を中止します。`);
    log('full_record が空のデータが多すぎます。Phase A の取り込みを確認してください。');
    process.exit(1);
  }
  ...
  log('\n→ 次: phase_d_posonly_fashard_fh06f.ts を実行して sim_position_only_stats を更新');
  log('=== Phase C 完了 ===');
}
