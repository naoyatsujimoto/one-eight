/**
 * import_sim_easy_s14.ts
 *
 * sim_easy_vs_easy_20260512e.md (10,000局) を取り込む。
 * sim_batch_id: easy_20260512_014
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   npx vite-node scripts/import_sim_easy_s14.ts
 *
 * フェーズ分割設計（タイムアウト耐性）:
 *   Phase A: sim_match_logs INSERT (10,000件)
 *     - 既存バッチが10,000件揃っていればスキップ → Phase B/C へ
 *     - 既存バッチが不完全（< 10,000件）→ 削除して再INSERT
 ...
 *   - sim_medium_pattern_stats は直接 upsert 方式で処理する
 */
...
const SIM_FILE_PATH = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_easy_vs_easy_20260512e.md';
...
async function main() {
  console.log('=== import_sim_easy_s14.ts ===');
  ...
  if (games.length !== EXPECTED_GAME_COUNT) {
    console.warn(`[WARN] 期待局数 ${EXPECTED_GAME_COUNT} に対し ${games.length} 局がパースされました`);
  }
  ...
  if (existing >= EXPECTED_GAME_COUNT) {
    ...
    // INSERT: 10,000件対応のため INSERT_BATCH_SIZE=5 で進める
    const INSERT_BATCH_SIZE = 5;
    const MAX_RETRY = 3;
    const PROGRESS_INTERVAL = 50; // 50バッチごとに進捗表示
    ...
    for (let i = 0; i < allRows.length; i += INSERT_BATCH_SIZE) {
      ...
      const batchIdx = Math.floor(i / INSERT_BATCH_SIZE);
      if (batchIdx % PROGRESS_INTERVAL === 0) {
  ...
  for (let i = 0; i < allRows.length; i++) {
    ...
    if ((bSuccess + bSkip + bError) % 200 === 0) {
