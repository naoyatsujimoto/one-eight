 ...
 *
 * sim_batch_id=fvhard_20260702_007 の sim_position_only_stats への差分 upsert。
 * phase_c_fvhard_007.ts 完了後に実行する。
 */
...
async function main() {
  ...
  if (!batchCount || batchCount === 0) {
    log('ERROR: バッチが sim_match_logs に存在しません。先に import_fvhard_007.ts を実行してください。');
    process.exit(1);
  }
