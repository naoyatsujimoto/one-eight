/**
 * import_sim_easy_s8.ts
 *
 * sim_easy_vs_easy_20260508g.md (5,000局) を取り込む。
 * sim_batch_id: easy_20260508_008
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   npx vite-node scripts/import_sim_easy_s8.ts
 *
 * フェーズ分割設計（タイムアウト耐性）:
 ...
 *   Phase C: sim_medium_pattern_stats upsert (Phase B 完了後)
 *
 * 制約:
...
const SIM_FILE_PATH = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_easy_vs_easy_20260508g.md';
const SIM_BATCH_ID = 'easy_20260508_008';
const SIM_POLICY = 'easy_vs_easy';
const SIM_VERSION = '1.0.0';
const ENGINE_VERSION = '1.0.0';
const RULES_VERSION = '1.1.0';
const GENERATED_AT = '2026-05-08T00:00:00Z';
...
async function main() {
  console.log('=== import_sim_easy_s8.ts ===');
  ...
  // ─── Phase C: sim_medium_pattern_stats upsert ────────────────────────────────

  console.log('--- Phase C: sim_medium_pattern_stats upsert ---');
  ...
  if (mediumTableMissing) {
    ...
    console.warn('Naoya が phase_medium_pattern.sql と phase_sim_medium_pattern_upsert.sql を SQL Editor で実行後、');
    console.warn('fix_sim_s8_medium.ts を実行してください。');
  } else {
    // RPC 存在確認（空配列でテスト）
    ...
    if (rpcMissing) {
      console.warn('[WARN] batch_upsert_sim_medium_pattern_stats RPC が存在しません。Phase C をスキップします。');
      console.warn('Naoya が phase_sim_medium_pattern_upsert.sql を SQL Editor で実行後、fix_sim_s8_medium.ts を実行してください。');
    } else {
      let cSuccess = 0;
      let cSkip = 0;
      let cError = 0;
      ...
      for (let i = 0; i < allRows.length; i++) {
        ...
        if (rpcErr) {
          if (rpcErr.message.includes('does not exist') || rpcErr.message.includes('function') || rpcErr.message.includes('Could not find')) {
            console.warn('[WARN] Phase C 実行中に RPC 不在エラー。中断します。');
            console.warn('Naoya が phase_sim_medium_pattern_upsert.sql を実行後、fix_sim_s8_medium.ts を実行してください。');
            break;
          }
          console.error(`  RPC ERROR (game ${i + 1}): ${rpcErr.message}`);
          cError++;
