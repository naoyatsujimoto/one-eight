/**
 * phase_d_fvhard_009.ts
 *
 * sim_batch_id=fvhard_20260806_009 の sim_position_only_stats への差分 upsert。
 * phase_c_fvhard_009.ts 完了後に実行する。
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   nohup node_modules/.bin/vite-node /tmp/phase_d_fvhard_009.ts > /tmp/phase_d_fvhard_009.log 2>&1 &
 *   tail -f /tmp/phase_d_fvhard_009.log
 */
...
async function main() {
  log('=== phase_d_fvhard_009.ts (Run 9) ===');
  ...
  // ── 他 policy 事前確認 ──
  const {count: fhPosBefore} = await supabase.from('sim_position_only_stats')
    .select('*',{count:'exact',head:true}).eq('sim_policy','fast_hard_vs_fast_hard');
  const {count: easyPosBefore} = await supabase.from('sim_position_only_stats')
    .select('*',{count:'exact',head:true}).eq('sim_policy','easy_vs_easy');
  log(`[事前] sim_position_only_stats(fast_hard): ${fhPosBefore}（変更しない）`);
  log(`[事前] sim_position_only_stats(easy_vs_easy): ${easyPosBefore}（変更しない）`);
  ...
  if (!batchCount || batchCount !== 10000) {
    log(`ERROR: バッチが正しく存在しません (${batchCount})。先に import_fvhard_009.ts を実行してください。`);
    process.exit(1);
  }
  ...
  while (true) {
    ...
    for (const row of data as {winner:string; full_record:{medium_pattern_id?:string}[]}[]) {
      ...
      const mids = row.full_record
        .map(m => m.medium_pattern_id)
        .filter((p): p is string => !!p);
      if (mids.length === 0) { skipCount++; continue; }

      // medium_pattern_id の ':' より前を position_only_id として抽出
      const posOnlyIds = mids
        .map(mid => mid.includes(':') ? mid.split(':')[0] : null)
        .filter((p): p is string => !!p);

      if (posOnlyIds.length === 0) { skipCount++; continue; }

      // 1局内で同じposition_only_idが複数出現しても1回だけ集計
  ...
  if (skipCount > 0) {
    log(`[WARN] skip=${skipCount} 件（winner未設定または full_record 空または medium_pattern_id なし）`);
  }
  ...
  if ((afterPos ?? 0) < (beforePos ?? 0)) {
    log(`[ERROR] 件数が減少しています！前回: ${beforePos} → 今回: ${afterPos}`);
    process.exit(1);
  }
  ...
  log(`\nRun 9 から抽出した unique position_only 数: ${newStats.size}`);
  log(`skip 数: ${skipCount}`);
  log(`upsert error 数: ${upsertErr}`);

  // ── 汚染チェック ──
  const {count: fhPosAfter} = await supabase.from('sim_position_only_stats')
    .select('*',{count:'exact',head:true}).eq('sim_policy','fast_hard_vs_fast_hard');
  const {count: easyPosAfter} = await supabase.from('sim_position_only_stats')
    .select('*',{count:'exact',head:true}).eq('sim_policy','easy_vs_easy');
  log(`\nsim_position_only_stats(fast_hard): ${fhPosAfter} (${fhPosBefore===fhPosAfter?'✅ 変化なし':'❌ 汚染'})`);
  log(`sim_position_only_stats(easy_vs_easy): ${easyPosAfter} (${easyPosBefore===easyPosAfter?'✅ 変化なし':'❌ 汚染'})`);
