/**
 * import_fashard_fh08.ts
 *
 * Fast Hard vs Fast Hard 10,000局 を sim_match_logs に取り込む。
 * sim_batch_id: fahard_20260520_008
 ...
 *   区切り: スペース（" "）
 *   passターン: "P,pass"
 *   ファイル: sim_fashard_vs_fashard_20260520h.md（Run 8 棋譜あり版）
 *
 * 設計方針:
 ...
 *   nohup node_modules/.bin/vite-node scripts/archive/import_fashard_fh08.ts > /tmp/fh08_import.log 2>&1 &
 *   tail -f /tmp/fh08_import.log
 *
 * 制約:
...
const SIM_FILE_PATH       = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_fashard_vs_fashard_20260520h.md';
...
async function main() {
  ...
  log('\n→ 次: phase_c_fashard_fh08.ts を実行して sim_medium_pattern_stats を更新');
  log('→ 次: phase_d_posonly_fashard_fh08.ts を実行して sim_position_only_stats を更新');
  log('=== Phase A 完了 ===');
}
