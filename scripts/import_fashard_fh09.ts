/**
 * import_fashard_fh09.ts
 *
 * Fast Hard vs Fast Hard 10,000局 を sim_match_logs に取り込む。
 * sim_batch_id: fahard_20260520_009
 ...
 *   区切り: スペース（" "）
 *   passターン: "P,pass"
 *   ファイル: sim_fashard_vs_fashard_20260520i.md（Run 9 棋譜あり版）
 *
 * 設計方針:
 *   - Phase A: sim_match_logs に UPSERT（conflict on sim_batch_id, game_index）
 *     ストリーミング処理（全量メモリ保持禁止）
 *   - Phase B: 廃止（sim_position_stats テーブル削除済み）
 *   - Phase C: 別スクリプト phase_c_fashard_fh06f.ts で実施
 *   - Phase D: 別スクリプト phase_d_posonly_fashard_fh06f.ts で実施
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   nohup node_modules/.bin/vite-node scripts/archive/import_fashard_fh09.ts > /tmp/fh09_import.log 2>&1 &
 *   tail -f /tmp/fh09_import.log
 *
 * 制約:
...
const SIM_FILE_PATH       = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_fashard_vs_fashard_20260520i.md';
const SIM_BATCH_ID        = 'fahard_20260520_009';
...
async function main() {
  ...
  log('\n→ 次: phase_c_fashard_fh09.ts を実行して sim_medium_pattern_stats を更新');
  log('→ 次: phase_d_posonly_fashard_fh09.ts を実行して sim_position_only_stats を更新');
  log('=== Phase A 完了 ===');
}
