// create_official_match_2345_naoya.ts
// naoya.tsujimoto@gmail.com (Black) vs oneeight-test@oneeightgame.com (White)
// starts_at: 本日 23:45 JST / entry_open: 23:30 JST
// timer_config: { mode: 'total_time', totalSeconds: 60 }
// Usage: npx tsx scripts/create_official_match_2345_naoya.ts
...
async function main() {
  ...
  // Step 2: starts_at = 本日 23:45 JST (UTC = 14:45)
  const nowJST = new Date(Date.now() + 9 * 60 * 60 * 1000);
  const jstDateStr = nowJST.toISOString().slice(0, 10); // YYYY-MM-DD (JST)
  // 23:45 JST = 14:45 UTC
  const startsAt    = new Date(`${jstDateStr}T14:45:00.000Z`);
  const entryOpenAt = new Date(startsAt.getTime() - 15 * 60 * 1000); // 23:30 JST = 14:30 UTC
  ...
  // Step 3: 重複チェック — 同日 23:45 JST・同参加者の scheduled match が既存かどうか
  const windowStart = new Date(startsAt.getTime() - 60 * 1000).toISOString(); // -1分
  const windowEnd   = new Date(startsAt.getTime() + 60 * 1000).toISOString(); // +1分
