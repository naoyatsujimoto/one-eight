/**
 * rebuild_sim_medium_s14.ts
 *
 * sim_medium_pattern_stats (easy_vs_easy) を全件 DELETE して
 * sim_match_logs 全60,000局から正しく再構築する。
 *
 * 修正点（s13比）:
 *   - 全件スキャンに ORDER BY sim_batch_id, game_index を付与（pagination 欠落防止）
 *   - DELETE も ORDER BY id で安定取得
 *
 * 制約:
...
async function main() {
  console.log('=== rebuild_sim_medium_s14.ts ===');
  console.log(`sim_policy: ${SIM_POLICY}`);
  console.log('ORDER BY: sim_batch_id, game_index (安定ページネーション)\n');
  ...
  console.log('--- Step 3: sim_match_logs スキャン & 集計 (ORDER BY sim_batch_id, game_index) ---');
  ...
  while (true) {
    const { data, error } = await supabase
      .from('sim_match_logs')
      .select('winner, full_record, sim_batch_id, game_index')
  ...
  while (true) {
    ...
    // ORDER BY medium_pattern_id で安定取得→削除
  ...
  for (let i = 0; i < allStats.length; i += UPSERT_CHUNK) {
    ...
    if ((insertedOk + insertErrors) % 10000 === 0) {
