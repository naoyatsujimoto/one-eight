/**
 * import_sim_easy_s5.ts
 *
 * sim_easy_vs_easy_20260508d.md をパースして sim_match_logs / sim_position_stats に取り込む。
 *
 * 実行方法:
 *   cd ~/Desktop/ONE_EIGHT/one-eight-web-mvp
 *   npx vite-node scripts/import_sim_easy_s5.ts
 *
 * 前提:
...
const SIM_FILE_PATH = '/Users/nt/Desktop/Claude_Cowork/sim_easy/sim_easy_vs_easy_20260508f.md';
const SIM_BATCH_ID = 'easy_20260508_007';
...
async function main() {
  console.log('=== import_sim_easy_s5.ts ===');
  ...
  if ((existingCount ?? 0) > 0) {
    console.log(`既存 sim_match_logs (batch_007): ${existingCount} 件（スキップ: INSERT不要）`);
    // FIX: エラーにしない、続行する
    // process.exit(1);
  }
  ...
  // 5. sim_match_logs に INSERT（FIX版: スキップ）
  console.log('sim_match_logs INSERT: スキップ（既存データあり）\n');
  // await insertSimMatchLogs(insertRows); // スキップ
  // skipped
