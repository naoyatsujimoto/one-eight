/**
 * phase_c_fvhard_008.ts
 *
 * sim_batch_id=fvhard_20260702b_008 の sim_medium_pattern_stats への差分 upsert。
 * import_fvhard_008.ts（Phase A）完了後に実行する。
 */
...
async function main() {
  ...
  if (skipCount > gameCount * 0.1) {
    log(`[WARN] skip率が高い: skip=${skipCount}/${gameCount+skipCount}`);
  }
  ...
  if (upsertErr > 0) {
    log(`[ERROR] Phase C upsert エラー ${upsertErr} 件 — 確認が必要`);
    process.exit(1);
  }
  ...
  const {count: easyMedAfter} = await supabase.from('sim_medium_pattern_stats')
    .select('*',{count:'exact',head:true}).eq('sim_policy','easy_vs_easy');
  log(`sim_medium_pattern_stats(fast_hard): ${fhMedAfter} (${fhMedBefore===fhMedAfter?'✅ 変化なし':'❌ 汚染'})`);
